...
> allPossibleVMoves (rows,columns) = [(row,column) | row <-[0..(rows*columns-1)], column <-[0..(rows*columns-1)] , (column-row) == columns, not (row == column)] 
...
> allPossibleHMoves (rows,columns) = [(row,column) | row <-[0..(rows*columns-1)], column <-[0..(rows*columns-1)] , (column-row) == 1, not (row == column), not ((row + 1) `mod` columns == 0)]

> allPoints :: BoardSize -> [Point] -- return all of the points available on an empty board of size BoardSize
> allPoints (rows,columns) = [0..(rows*columns-1)]

> possibleVMoves :: Board -> [Move] 
> possibleVMoves (Board _ (rows,columns) _ vmoves points movesMade _) = [(row,column) | row <-points, column <-points , (column-row) == columns, not (row == column)]

> possibleHMoves :: Board -> [Move] -- return all of the possible horizonal moves available on an empty board with size BoardSize. 
> possibleHMoves (Board _ (rows,columns) hmoves _ points movesMade _) = [(row,column) | row <-points, column <-points , (column-row) == 1, not (row == column), not ((row + 1) `mod` columns == 0)]
...
> initialBoard :: Board
> initialBoard = let size = (3,3) in Board H size (allPossibleHMoves size) (allPossibleVMoves size) (allPoints size) [] 0

> boardExample :: Board
> boardExample = let size = (3,3) in Board H size [(0,1), (1,2)] [(0,3),(3,6),(2,5),(5,8)] [0,1,2,3,5,6,8] [(4,7)] 0
...
apply the move to the board and check the possible horizontal moves hms'

> play m (Board H size hms vms points mvsMade 0) = let points' = (delete (snd m) (delete (fst m) points)) in --update the points so that both of the points in the move are deleted from the list of available points.
>                                          let board = Board H size hms vms points' (insert m mvsMade) 0 in --update the board so that it is represented if the move was made.
>                                          let hms' = possibleHMoves board in 
>                                          let vms' = possibleVMoves board in
>                                          if vms' == [] --check if the vertical player has any moves left, if not then horizontal player has won
>                                          then Board V size hms' vms' [] (insert m mvsMade) 1
>                                          else Board V size hms' vms' points' (insert m mvsMade) 0
> play m (Board V size hms vms points mvsMade 0) = let points' = (delete (snd m) (delete (fst m) points)) in --update the points so that both of the points in the move are deleted from the list of available points.
>                                          let board = Board V size hms vms points' (insert m mvsMade) 0 in --update the board so that it is represented if the move was made.
>                                          let hms' = possibleHMoves board in 
>                                          let vms' = possibleVMoves board in
>                                          if vms' == [] --check if the vertical player has any moves left, if not then horizontal player has won
>                                          then Board H size hms' vms' [] (insert m mvsMade) 1
>                                          else Board H size hms' vms' points' (insert m mvsMade) 0
...
> playMoves _ board@(Board _ _ [] _ _ _ _)     = Just board --if the horizontal player has no more moves left, then they have lost, so just return the board.
> playMoves _ board@(Board _ _ _ [] _ _ _)     = Just board --if the vertical player has no more moves left, then they have lost, so just return the board.
> playMoves (mv:mvs) board@(Board H _ hms vms points mvsMade _) = if not (elem mv hms) then Nothing
>                         else playMoves mvs (play mv board)
> playMoves (mv:mvs) board@(Board V _ hms vms points mvsMade _) = if not (elem mv vms) then Nothing
>                         else playMoves mvs (play mv board)
...
playMoves [(1,2),(3,6)] initialBoard 
--Just O plays next
...
set the heuristic to be the difference between the hmoves and the vmoves, if there are more hmoves then Horizontal is more likely to win. 
Use alpha beta pruning to improve efficiency.

At the beginning of the game do not search all of the tree, only when you reach a point, when the tree is smaller, then you can search the whole of the tree.

Use maxBound and minBound for the initial values for the heuristic. Heuristic function should work out the difference between the moves for h and v and set the outcome value to represent that. 